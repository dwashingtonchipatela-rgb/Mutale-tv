document.addEventListener('DOMContentLoaded', async function() {
  ...
  ['click','keypress','scroll','mousemove'].forEach(function(e) { document.addEventListener(e, resetTimer); });
  resetTimer();
  ...
  uploadTabs.forEach(function(tab) {
    tab.addEventListener('click', function() {
      ...
      uploadPanels.forEach(function(p) { p.classList.remove('active'); });
  ...
  if (videoUrlInput && videoPreview) {
    videoUrlInput.addEventListener('input', function() {
      const match = this.value.trim().match(/(?:youtube\.com\/watch\?v=|youtu\.be\/)([^&\n?#]+)/);
      if (match) {
        ...
        videoPreview.innerHTML = '<p style="color:var(--admin-text-muted);text-align:center;padding:2rem;">Paste a YouTube URL to see preview</p>';
      }
    });
  }
  ...
  if (uploadPaperBtn && window.ZedLearnDB) {
    uploadPaperBtn.addEventListener('click', async function() {
      ...
      if (!title || !subject || !grade || !syllabus || !fileUrl) { alert('Please fill in all required fields.'); return; }
      ...
      this.disabled = true;
      const success = await window.ZedLearnDB.insertPastPaper({ title, subject, grade, syllabus, year: year ? parseInt(year) : null, file_url: safeUrl, is_premium: isPremium });
      if (success) {
        await window.ZedLearnDB.addNotification('New past paper added: ' + title, 'paper');
  ...
  if (uploadScriptBtn && window.ZedLearnDB) {
    uploadScriptBtn.addEventListener('click', async function() {
      ...
      if (!title || !subject || !grade || !fileUrl) { alert('Please fill in all required fields.'); return; }
      const safeUrl = window.ZedLearnSecurity ? window.ZedLearnSecurity.sanitizeURL(fileUrl) : fileUrl;
      if (safeUrl === '#') { alert('Please enter a valid URL.'); return; }
      this.textContent = 'Saving...';
      this.disabled = true;
      const success = await window.ZedLearnDB.insertLessonScript({ title, subject, grade, file_url: safeUrl, description });
      if (success) {
  ...
  if (uploadBookBtn && window.ZedLearnDB) {
    uploadBookBtn.addEventListener('click', async function() {
      ...
      if (!title || !type || !fileUrl) { alert('Please fill in all required fields.'); return; }
      const safeUrl = window.ZedLearnSecurity ? window.ZedLearnSecurity.sanitizeURL(fileUrl) : fileUrl;
      if (safeUrl === '#') { alert('Please enter a valid URL.'); return; }
      this.textContent = 'Saving...';
      this.disabled = true;
      const success = await window.ZedLearnDB.insertBook({ title, author, book_type: type, grade, subject, file_url: safeUrl });
      if (success) {
  ...
  if (uploadVideoBtn && window.ZedLearnDB) {
    uploadVideoBtn.addEventListener('click', async function() {
      ...
      if (!title || !subject || !grade || !url) { alert('Please fill in all required fields.'); return; }
      if (!url.includes('youtube.com') && !url.includes('youtu.be')) { alert('Please enter a valid YouTube URL.'); return; }
      this.textContent = 'Saving...';
      this.disabled = true;
      const success = await window.ZedLearnDB.insertVideo({ title, subject, grade, youtube_url: url, description });
      if (success) {
        ...
        if (videoPreview) videoPreview.innerHTML = '<p style="color:var(--admin-text-muted);text-align:center;padding:2rem;">Paste a YouTube URL to see preview</p>';
      } else { alert('Failed to save. Please try again.'); }
  ...
  if (saveLinksBtn && window.ZedLearnDB) {
    saveLinksBtn.addEventListener('click', async function() {
      ...
      if (!allOk) alert('Some links failed to save.');
  ...
  if (commentsTableBody && window.ZedLearnDB) {
    const reviews = await window.ZedLearnDB.getPendingReviews();
    const countEl = document.getElementById('pending-count');
    if (countEl) countEl.textContent = reviews.length;
    if (reviews.length === 0) {
      commentsTableBody.innerHTML = '<tr><td colspan="5" style="text-align:center;padding:3rem;color:var(--admin-text-muted);">No pending reviews.</td></tr>';
    } else {
      commentsTableBody.innerHTML = reviews.map(function(r) {
  ...
  if (changePassBtn && window.ZedLearnDB) {
    changePassBtn.addEventListener('click', async function() {
      ...
      if (newPass.length < 12) { alert('Password must be at least 12 characters.'); return; }
      if (newPass !== confirmPass) { alert('Passwords do not match.'); return; }
      this.textContent = 'Changing...';
      this.disabled = true;
      const success = await window.ZedLearnDB.changeAdminPassword(newPass);
      if (success) { alert('Password changed successfully.'); document.getElementById('new-password').value = ''; document.getElementById('confirm-password').value = ''; }
...
async function approveReviewAction(id) {
  if (window.ZedLearnDB) { const ok = await window.ZedLearnDB.approveReview(id); if (ok) location.reload(); }
}

async function deleteReviewAction(id) {
  if (confirm('Reject and delete this review?')) {
    if (window.ZedLearnDB) { const ok = await window.ZedLearnDB.deleteReview(id); if (ok) location.reload(); }
  }
}
